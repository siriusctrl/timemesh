...
test("opens the agent skill and switches appearance", async ({ page }) => {
  ...
  const revealState = await page.locator("[data-theme-reveal]").evaluate((element) => {
    ...
    const animation = element.querySelector("[data-theme-reveal-circle]")?.getAnimations()[0];
    const state = {
      animationId: animation?.id,
      keyframeCount: animation?.effect instanceof KeyframeEffect
        ? animation.effect.getKeyframes().length
        : undefined,
