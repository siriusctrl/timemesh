...
Planning should apply the transport composition rules in section 8: de-duplicate repeated unlabeled participant tokens and repeated `(label, token)` pairs, while preserving identical token text carrying distinct labels as distinct responses. Transport input may repeat the same base token when several complete response bundles are concatenated; normalize identical copies and reject distinct bases.
...
An optional display name may prefix a token line with a pipe delimiter:
...
Labels are not part of Token v2 and do not affect checksums or base references. A transport parser should preserve them for display, normalize repeated `(label, token)` pairs, count identical token text with distinct labels as distinct responses, and reject one label attached to multiple distinct participant tokens. Unlabeled duplicate participant tokens remain de-duplicated.
